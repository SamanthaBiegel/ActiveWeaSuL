...
class TrainConfig(Config):
    ...
    s
        Weak label model parameter regularization strength
    optimizer
...
class SnorkelLabelModel(nn.Module, BaseLabeler):
    ...
    **kwargs
    ...
    def _init_params(self) -> None:
        ...
        if isinstance(self.train_config.prec_init, (int, float)):
            ...
            self._prec_init = self.train_config.prec_init
    ...
    def predict(
        ...
        array([0, 1, 0])
        """
        return super(SnorkelLabelModel, self).predict(L, return_probs, tie_break_policy)
    ...
    def score(
        ...
        {'f1': 0.8}
        """
        return super(SnorkelLabelModel, self).score(L, Y, metrics, tie_break_policy)
    ...
    def _loss_pk(self, s: float = 0.0) -> torch.Tensor:
        r"""Loss from prior knowledge"""

        m = torch.ones([self.d])
        m[:-self.cardinality] = 0

        mu_known = torch.zeros([self.d, self.cardinality])
        for i, k in enumerate(range(self.d - self.cardinality, self.d)):
            mu_known[k, i] = 1

        return torch.norm(s * torch.diag(m) @ (mu_known - self.mu)) ** 2

    def _loss_mu(self, l2: float = 0, s: float = 0.0) -> torch.Tensor:
        r"""Overall mu loss.
        ...
        return loss_1 + loss_2 + self._loss_l2(l2=l2) + self._loss_pk(s)
    ...
    def fit(
        ...
        **kwargs
        ...
        for epoch in range(start_iteration, self.train_config.n_epochs):
            ...
            loss = self._loss_mu(l2=self.train_config.l2, s=self.train_config.s)
            if torch.isnan(loss):
