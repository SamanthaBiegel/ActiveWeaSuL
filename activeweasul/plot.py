...
class PlotMixin:
    ...
    def plot_parameters(self, true_values="max cliques"):
        ...
        true_mu_df = pd.DataFrame(
            np.repeat(
                self.label_model.get_true_mu().detach().numpy()[:, 1][None, :],
                self.it + 1, axis=0))
    ...
    def plot_sampled_points(self):
        ...
        fig = go.Figure(go.Scatter(
            x=list(range(1, self.it + 1)), y=conf_list, mode="markers", marker=dict(
                size=8, line_width=0.2), text=self.y[self.queried], name="Sampled points"))
        fig.update_layout(
            height=600, template="plotly_white", xaxis_title="Active Learning Iteration",
            yaxis_title="WL Configuration")
        return fig
    ...
    def plot_sampled_classes(self):
        ...
        for i in self.label_model.y_set:
            fig.add_trace(go.Scatter(
                x=np.array(list(range(self.it)))[self.y[self.queried] == i] + 1,
    ...
    def plot_metrics(self, metrics, true_label_counts=False):
        ...
        if true_label_counts:
            metric_dict = self.label_model._analyze(
                self.label_model.predict_true_counts(), self.df["y"].values)
        else:
            metric_dict = self.label_model._analyze(
                self.label_model.predict_true(), self.df["y"].values)
    ...
    def plot_iterations(self):
        """Plot sampled weak label configurations, ground truth labels and resulting
        probabilistic labels over time"""
    ...
    def _color_df(self, df):
        ...
        idx = np.where(
            (
                self.first_labels != self.df["y"]
            ) & self.second_labels == self.df["y"]
        )
