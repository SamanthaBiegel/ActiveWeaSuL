...
class ActiveLearningQuery():
    """Sample a data point from unlabeled pool following an active
    learning strategy, based on probabilistic label output.
    ...
    def margin_strategy(self):
        ...
        return np.where(
            np.logical_and(
                bucket_margins == np.min(bucket_margins[self.is_valid_bucket]),
