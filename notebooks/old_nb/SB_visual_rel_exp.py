...
#     abs_diff = np.abs(Y_probs[:,0] - Y_probs[:,1]) + np.abs(Y_probs[:,0] - Y_probs[:,2]) + np.abs(Y_probs[:,1] - Y_probs[:,2])
    Y_probs_sorted = np.sort(Y_probs, axis=1)
    abs_diff = np.abs(Y_probs_sorted[:,2] - Y_probs_sorted[:,1])
...
(Y_true == second_labels).sum()

second_probs[:,Y_true]

mask = np.zeros((len(Y_true), 3))
mask[:,0] = Y_true==0
mask[:,1] = Y_true==1
mask[:,2] = Y_true==2

second_probs[mask.astype(bool)] - first_probs[mask.astype(bool)]
